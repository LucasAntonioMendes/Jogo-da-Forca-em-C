...
void jogar (){
    ...
    for(i=0;i<strlen(guarda.palavra); i++){
        if(guarda.palavra[i] == 181 || guarda.palavra[i] == 183 || guarda.palavra[i] == 199 || guarda.palavra[i] == 142){
            palavraEdit[i]=65;
...
void adicionarDica(){
    ...
    do{
        system("cls");
            printf("\nDIGETE UMA DICA DE ATE 200 CACTERES");
            fflush(stdin);
            scanf("%200[^\n]", guarda.dica);
            printf("DESEJA CONTINUAR COM A DICA %s?<S/N>",guarda.dica);
            fflush(stdin);
            resp= getchar();
    }while (resp!='S' && resp !='s');
    FILE *arq;
